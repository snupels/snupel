...
const articles = [
  ...
  ["제6조 회원정보 및 계정 관리", "회원은 정확한 정보를 유지하고 변경 시 수정해야 하며 계정과 로그인 정보를 안전하게 관리해야 합니다. 계정 도용이나 부정사용을 인지하면 운영정보와 고객지원 화면의 공개 문의 채널로 알려야 합니다."],
  ...
  ["제15조 회원 탈퇴", "회원은 계정 화면의 회원 탈퇴·개인정보 요청 안내를 통해 언제든 탈퇴를 요청할 수 있습니다. 탈퇴 시 패스포트, 미션, 스탬프, 저장 일정과 공개 피드 정보가 삭제될 수 있으며 개인정보는 관계 법령과 개인정보 처리방침에 따라 처리됩니다."],
...
export function ServiceTermsPage() {
  return <TermsLayout title="이용약관"><p className="mt-4 leading-7 text-[#5f6c65]">강원 스포츠 패스포트 서비스 이용에 필요한 조건과 권리·의무를 확인해 주세요.</p><div className="mt-8 space-y-6">{articles.map(([title, body]) => <section key={title}><h2 className="font-bold text-[#24342b]">{title}</h2><p className="mt-2 leading-7 text-[#5f6c65]">{body}</p></section>)}</div><p className="mt-8 rounded-xl bg-[#f3f7f4] px-4 py-3 text-sm">부칙: 본 약관은 2026년 8월 30일부터 시행합니다.</p><Link href="/support/" className="mt-4 inline-flex text-sm font-bold text-[#008f45] underline">운영정보와 고객지원 확인</Link></TermsLayout>;
}

export function PrivacyTermsPage() {
  return <TermsLayout title="개인정보 수집·이용 동의"><p className="mt-4 leading-7 text-[#5f6c65]">회원가입과 패스포트 서비스 제공을 위해 아래 개인정보를 수집·이용합니다.</p><div className="mt-8 overflow-hidden rounded-2xl border border-[#dfe5e1]"><dl className="divide-y divide-[#e5eae7] text-sm"><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">수집·이용 목적</dt><dd>회원가입 및 회원관리, 이용자 식별, 패스포트 서비스 제공, 스포츠 활동·미션·이벤트 참여와 인증 관리, 문의 처리 및 서비스 안내</dd></div><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">필수 수집 항목</dt><dd>아이디, 이메일, 로그인 계정 정보, 닉네임, 전화번호</dd></div><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">선택 입력 항목</dt><dd>프로필 사진, 생년월일, 성별, 주소(우편번호·기본주소·상세주소)</dd></div><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">자동 생성 정보</dt><dd>접속·서비스 이용기록, 미션·이벤트 참여정보, 인증내역, 스탬프, 관심 콘텐츠 및 저장 일정</dd></div><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">보유 기간</dt><dd>회원 탈퇴 시까지. 관계 법령에 따라 보존이 필요한 경우 해당 기간까지 보관</dd></div></dl></div><p className="mt-5 text-sm leading-6 text-[#5f6c65]">필수 개인정보 수집·이용 동의를 거부할 수 있으나, 동의하지 않으면 회원가입과 회원 대상 서비스 이용이 제한됩니다. 주소는 선택 입력으로, 회원 주소 정보 관리에 이용하며 입력하지 않아도 가입할 수 있습니다. 공개 프로필이나 스포츠 피드에는 표시하지 않으며, 마이페이지에서 언제든 수정하거나 비우고 저장하여 삭제할 수 있습니다.</p></TermsLayout>;
}

export function MarketingEmailTermsPage() {
  return <TermsLayout title="이메일 마케팅 수신 동의"><p className="mt-4 leading-7 text-[#5f6c65]">강원 스포츠 패스포트의 행사와 혜택 정보를 이메일로 받아볼 수 있습니다.</p><div className="mt-8 overflow-hidden rounded-2xl border border-[#dfe5e1]"><dl className="divide-y divide-[#e5eae7] text-sm"><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">이용 목적</dt><dd>스포츠 행사·이벤트·축제, 신규 콘텐츠, 관광정보 및 혜택 안내</dd></div><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">이용 항목</dt><dd>이메일 주소</dd></div><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">보유 기간</dt><dd>회원 탈퇴 또는 수신 동의 철회 시까지</dd></div></dl></div><p className="mt-5 text-sm leading-6 text-[#5f6c65]">동의는 선택사항이며 동의하지 않아도 기본 서비스를 이용할 수 있습니다. 나의 패스포트의 계정 정보에서 언제든 변경할 수 있습니다.</p></TermsLayout>;
}

export function MarketingSmsTermsPage() {
  return <TermsLayout title="SMS 마케팅 수신 동의"><p className="mt-4 leading-7 text-[#5f6c65]">강원 스포츠 패스포트의 행사와 혜택 정보를 SMS로 받아볼 수 있습니다.</p><div className="mt-8 overflow-hidden rounded-2xl border border-[#dfe5e1]"><dl className="divide-y divide-[#e5eae7] text-sm"><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">이용 목적</dt><dd>스포츠 행사·이벤트·축제, 프로그램 및 프로모션 안내</dd></div><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">이용 항목</dt><dd>전화번호</dd></div><div className="grid gap-2 p-4 sm:grid-cols-[150px_1fr]"><dt className="font-bold">보유 기간</dt><dd>회원 탈퇴 또는 수신 동의 철회 시까지</dd></div></dl></div><p className="mt-5 text-sm leading-6 text-[#5f6c65]">동의는 선택사항이며 동의하지 않아도 기본 서비스를 이용할 수 있습니다. 나의 패스포트의 계정 정보에서 언제든 변경할 수 있습니다.</p></TermsLayout>;
}
