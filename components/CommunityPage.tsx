...
type FeedTab = "all" | "mine" | "following";
...
function CommunityPageContent() {
  ...
  const loadFeed = useCallback(async (nextTab: FeedTab, nextPage = 1) => {
    ...
    try {
      ...
      setPage(nextPage); setHasMore(result.length === 20);
      ...
      <section className="border-b border-[#dfe8e2] bg-white px-4 py-10 sm:px-6">
        <div className="mx-auto flex max-w-[1120px] flex-col gap-7 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold tracking-[-0.04em] sm:text-4xl">{viewingProfile ? `${profiles[profileUserId]?.name ?? posts[0]?.authorName ?? "사용자"}님의 스포츠 피드` : "강원 스포츠 피드"}</h1>
      ...
      <div className="mx-auto max-w-[1120px] px-4 pt-8 sm:px-6">
        ...
        {tab !== "all" && !viewingProfile && !user ? (
          ...
          <section className="mt-8 rounded-[24px] border border-dashed border-[#cbd9d0] bg-white/70 px-6 py-16 text-center">
            ...
            <h2 className="mt-5 text-xl font-bold">{tab === "following" ? "팔로우한 탐험가의 게시글이 여기에 모여요" : "아직 공개된 인증 사진이 없어요"}</h2>
          ...
          <section className="mt-8 grid items-start gap-5 sm:grid-cols-2 xl:grid-cols-4">
              ...
              return (
                <article key={post.id} className="overflow-hidden rounded-[22px] border border-[#dde6e0] bg-white shadow-[0_4px_16px_rgba(23,58,45,0.07)]">
                  ...
                  <div className="relative aspect-square overflow-hidden bg-[#e7ece8]">
                    {post.proofUrl ? <Image src={post.proofUrl} alt={post.isDemo ? "운영자 데모 안내 이미지" : `${post.placeName ?? "강원 스포츠"} 인증 사진`} fill preload={index === 0} sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 25vw" className="object-cover" /> : <div className="flex h-full items-center justify-center text-[#819087]"><AppIcon name="instagram" className="size-10" /></div>}
                  ...
                  <div className="p-4">
                    ...
                    {tab === "mine" && !post.isDemo && <button type="button" onClick={() => void hideFromFeed(post.id)} className="mt-4 cursor-pointer text-xs font-semibold text-[#929c96] underline-offset-2 hover:text-[#b43d3d] hover:underline">피드에서 숨기기</button>}
                  </div>
                </article>
              );
            })}
