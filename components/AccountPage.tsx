        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
          ...
          <form onSubmit={submit} className="rounded-[24px] border border-[#dfe7e1] bg-white p-6 shadow-sm sm:p-8">
            ...
            <div className="mt-7 space-y-5">
              ...
              <fieldset className="rounded-2xl border border-[#dce4df] bg-[#f8faf9] p-4"><legend className="px-1 text-sm font-bold">홍보 수신 설정</legend><label className="mt-2 flex items-center gap-2 text-sm"><input name="agreeMarketingEmail" type="checkbox" defaultChecked={user.marketingEmailAgreed} className="size-4 accent-[#008f45]" /><span>이메일 마케팅 수신 <button type="button" onClick={() => setConsentDocument("marketingEmail")} className="ml-1 text-[#008f45] underline">보기</button></span></label><label className="mt-3 flex items-center gap-2 text-sm"><input name="agreeMarketingSns" type="checkbox" defaultChecked={user.marketingSnsAgreed} className="size-4 accent-[#008f45]" /><span>SMS 마케팅 수신 <button type="button" onClick={() => setConsentDocument("marketingSms")} className="ml-1 text-[#008f45] underline">보기</button></span></label><div className="mt-3 flex gap-3"><button type="button" onClick={() => setConsentDocument("service")} className="text-xs font-semibold text-[#008f45] underline">이용약관 보기</button><button type="button" onClick={() => setConsentDocument("privacy")} className="text-xs font-semibold text-[#008f45] underline">개인정보 동의 보기</button></div></fieldset>
            </div>
