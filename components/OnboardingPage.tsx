...
import { loginHref, safeReturnPath } from "@/lib/auth-flow";
...
export function OnboardingPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.replace(next);
    } catch { if (isCurrent()) setError("가입 정보를 저장하지 못했습니다. 전화번호와 입력 내용을 확인해 주세요."); }
    finally { if (isCurrent()) setPending(false); }
  }
