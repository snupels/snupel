...
export function SportsLocationMap({ activity }: { activity: ActivityResponse }) {
  ...
  const drawMap = useCallback(() => {
    ...
    if (!activity.address) {
      ...
      new maps.Map(container, { center, level: 10 });
      return;
    }
  ...
  return (
    <section className="mt-8">
      ...
      <div className="relative h-[300px] overflow-hidden rounded-2xl border border-[#dce6df] bg-[#eaf1ec] shadow-sm sm:h-[360px]">
        ...
        <Link href={mapHref} aria-label={`${title}를 전체 지도에서 보기`} className="absolute inset-0 z-10 cursor-pointer focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-inset focus-visible:ring-[#008f45]">
          <span className="absolute right-4 top-4 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-[#172033] shadow-lg">지도에서 보기<AppIcon name="arrowRight" /></span>
        </Link>
